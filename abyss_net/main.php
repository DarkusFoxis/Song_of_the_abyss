...
switch ($filter) {
    case 'top':
        $sql = "SELECT p.*, u.username, u.avatar FROM post p JOIN users u ON p.id_user = u.id WHERE p.total_rating > 0 ORDER BY p.total_rating DESC LIMIT $posts_per_page OFFSET $offset";
        $total_posts_query = "SELECT COUNT(*) FROM post";
        break;
    case 'unrated':
        $sql = "SELECT p.*, u.username, u.avatar FROM post p JOIN users u ON p.id_user = u.id WHERE p.total_rating = 0 ORDER BY UNIX_TIMESTAMP(p.data) DESC LIMIT $posts_per_page OFFSET $offset";
        $total_posts_query = "SELECT COUNT(*) FROM post WHERE total_rating = 0";
        break;
    case 'abyss':
        $sql = "SELECT p.*, u.username, u.avatar FROM post p JOIN users u ON p.id_user = u.id WHERE p.total_rating < 0 ORDER BY p.total_rating ASC LIMIT $posts_per_page OFFSET $offset";
        $total_posts_query = "SELECT COUNT(*) FROM post";
        break;
    default:
        $sql = "SELECT p.*, u.username, u.avatar FROM post p JOIN users u ON p.id_user = u.id ORDER BY UNIX_TIMESTAMP(p.data) DESC LIMIT $posts_per_page OFFSET $offset";
        $total_posts_query = "SELECT COUNT(*) FROM post";
        break;
}
            ...
            <div class="post-card">
                ...
                <div class="rating-container">
                    <div class="rating-buttons">
                        <button id="btn-plus-<?php echo $row['id_post']; ?>" class="rating-btn rating-btn-plus" onclick="ratePost(<?php echo $row['id_post']; ?>, 1)">▲</button>
                        <button id="btn-minus-<?php echo $row['id_post']; ?>" class="rating-btn rating-btn-minus" onclick="ratePost(<?php echo $row['id_post']; ?>, -1)">▼</button>
                    </div>
                    <div class="rating-value"><span id="rating-<?php echo $row['id_post']; ?>"><?php echo round($row['total_rating'], 2); ?></span></div>
                </div>
            </div>
            <?php
        }
...
function loadMorePosts() {
    ...
    $.ajax({
        url: '?page=' + currentPage + '&ajax=1&filter=' + currentFilter,
        type: 'GET',

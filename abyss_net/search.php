...
<head>
    ...
    <title><?php echo (isset($_GET['q']) && !empty(trim($_GET['q']))) ? 'Abyss Search: "' . htmlspecialchars($_GET['q']) . '"' : 'Abyss Search'; ?></title>
    ...
    <style>
        ...
        input:checked + .safe-search-slider {
            background-color: #2ecc71;
        }
        input:checked + .safe-search-slider:before {
    ...
    <div class="search-hero">
        ...
        <div class="search-container">
            <form id="searchForm" method="GET" action="">
                <div class="form-group" style="position:relative;">
                    <div class="search-input-container">
                        <input type="text" name="q" id="searchInput" class="form-control search-input" placeholder="Введите ваш запрос..." value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>" autocomplete="off" autofocus>
                        <button type="button" id="clearSearch" title="Очистить поиск">&times;</button>
                    </div>
                    <div id="suggestions"></div>
                </div>
                 <div class="safe-search-container">
                    <label class="safe-search-label" id="safeSearchLabel" for="safeSearchToggle">
                        <span id="safeSearchText">Безопасный поиск</span>
                        <span class="safe-search-toggle">
                            <input type="checkbox" id="safeSearchToggle">
                            <span class="safe-search-slider"></span>
                        </span>
                    </label>
                </div>
            ...
            <div class="results-container">
                ...
                $safeSearchEnabled = isset($_GET['safe']) ? (bool)$_GET['safe'] : true;
                    ...
                    if ($stmt) {
                        ...
                        if ($numResults > 0) {
                            ...
                            if ($totalPages > 1) {
                                ...
                                if ($page > 1) {
                                    $prevPageUrl = 'javascript:loadPage('.($page-1).')';
                                    echo '<a href="'.$prevPageUrl.'">← Назад</a>';
                                }
                                if ($page < $totalPages) {
                                    $nextPageUrl = 'javascript:loadPage('.($page+1).')';
                                    echo '<a href="'.$nextPageUrl.'">Вперёд →</a>';
                                }
                                echo '</div>';
                            }
                        } else {
...
$(document).ready(function() {
    ...
    const safeSearchToggle = $('#safeSearchToggle');
    const safeSearchLabel = $('#safeSearchLabel');
    const safeSearchText = $('#safeSearchText');

    const savedSafeSearch = localStorage.getItem('safeSearch');
    const initialSafeSearch = savedSafeSearch !== null ? savedSafeSearch === 'true' : true;
    safeSearchToggle.prop('checked', initialSafeSearch);
    if (initialSafeSearch) {
        safeSearchLabel.addClass('active');
    }
    ...
    clearSearchBtn.on('click', function() {
        ...
        clearSearchBtn.hide();
        // Загрузить "главную" страницу поиска (без запроса)
        loadResults('', 1, safeSearchToggle.is(':checked'));
        // Обновить URL
        const newUrl = 'search'; 
        history.pushState({ q: '', page: 1, safe: safeSearchToggle.is(':checked') }, '', newUrl);
        document.title = 'Abyss Search';
    });
    ...
    function loadResults(query, page, safeSearch) {
        const data = { q: query, page: page, safe: safeSearch ? 1 : 0 };
        $.ajax({
            url: 'search',
            method: 'GET',
            data: data,
            success: function(data) {
                ...
                if (!safeSearch) newUrl += '&safe=0';
                history.pushState({ q: query, page: page, safe: safeSearch }, '', newUrl);
                if (query.trim() !== '') {
                     document.title = 'Abyss Search: "' + query + '"';
                } else {
                    document.title = 'Abyss Search';
                }
            }
    ...
    $('#searchForm').submit(function(e) {
        ...
        const safeSearch = safeSearchToggle.is(':checked');
        if (query || query === '') {
            loadResults(query, 1, safeSearch);
        }
    });
    ...
    window.loadPage = function(page) {
        const query = searchInput.val().trim();
        const safeSearch = safeSearchToggle.is(':checked');
        loadResults(query, page, safeSearch);
    };
    ...
    safeSearchToggle.on('change', function() {
        const isChecked = $(this).is(':checked');
        localStorage.setItem('safeSearch', isChecked);
        if (isChecked) {
            safeSearchLabel.addClass('active');
        } else {
            safeSearchLabel.removeClass('active');
        }
        const query = searchInput.val().trim();
        loadResults(query, 1, isChecked);
    });

    window.addEventListener('popstate', function(event) {
        const state = event.state || { q: '', page: 1, safe: initialSafeSearch };
        const query = state.q || '';
        const page = parseInt(state.page) || 1;
        const safe = typeof state.safe !== 'undefined' ? state.safe : initialSafeSearch;

        searchInput.val(query);
        safeSearchToggle.prop('checked', safe);
        if (safe) {
            safeSearchLabel.addClass('active');
        } else {
            safeSearchLabel.removeClass('active');
        }
        loadResults(query, page, safe);
    });
});
</script>
</body>

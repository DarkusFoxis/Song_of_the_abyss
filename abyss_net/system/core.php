...
function display_upload_form($user_lvl, $user_id, $user_login, $username, $cooldown_active = false) {
    ...
    <div class="upload-form-container">
        ...
        <form id="uploadForm" action="upload_handler.php" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="audio_file">Аудиофайл (mp3, wav, ogg, webm):</label>
                <input type="file" id="audio_file" name="audio_file" class="form-control-file" accept=".mp3,.wav,.ogg,.webm" required {$disabled_attr}>
                <p style="font-size: 0.8em; color: #888; margin-top: 5px;"><em>Аудио будет автоматически сжато на сервере (экономия до 60+%)</em></p>
HTML;
    if ($can_upload_cover) {
        $form_html .= <<<HTML
            <div class="form-group">

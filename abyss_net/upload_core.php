...
if (($current_time - $last_post_time) < $etherTime) {
    $etherAdd = false;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mb_strlen($post) > ($user['lvl'] >= 3 ? 6000 : 3000)) {
        echo "Ошибка: Текст поста слишком длинный (максимум " . ($user['lvl'] >= 3 ? 6000 : 3000) . " символов).";
        exit;
    }
    ...
    if ($edit_id) {
        ...
        if ($media_names !== NULL) {
            $media_names = mysqli_real_escape_string($conn, $media_names);
        }
        $sql = "UPDATE post SET title='$title', post='$post', media=" . ($media_names !== NULL ? "'$media_names'" : "NULL") . " WHERE id_post=$edit_id";
        ...
        $media_names = !empty($uploaded_files) ? mysqli_real_escape_string($conn, implode(',', $uploaded_files)) : NULL;
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            $tonen_add_sql = "UPDATE `users` SET `abyss_ether`= `abyss_ether` + 0.0000001 WHERE `id` = $userId";
            $tonen_sql = "UPDATE `abyss_ether` SET `count`= `count` - 0.0000001 WHERE `id` = 1";
            if (mysqli_query($conn, $query_sql)) {
                echo "Успешно опубликованно!";
                if ($etherAdd) {
                    mysqli_query($conn, $tonen_add_sql);
                    mysqli_query($conn, $tonen_sql);
                }
            } else {
                echo "Ошибка: " . mysqli_error($conn);
            }

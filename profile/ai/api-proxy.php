...
function logApiRequest($user_login, $provider, $model, $system_prompt, $request_messages, $response_content, $usage = null, $stream_mode = false) {
    $log_dir = __DIR__;
    ...
    $log_data = [
        ...
        'request_messages_count' => $request_messages,
        'response_preview' => $response_content ? $response_content : 'STREAMING_RESPONSE',
        'response_length' => $response_content ? strlen($response_content) : 0,
        'usage' => $usage,
        'stream_mode' => $stream_mode
    ];

    $log_line = json_encode($log_data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . "\n" . str_repeat('-', 80) . "\n";
...
if ($stream) {
    ...
    if ($http_code >= 200 && $http_code < 300) {
        logApiRequest(
            ...
            $api_messages,
            $full_response,
            ['streaming' => true, 'response_length' => strlen($full_response)],
            true
        );
    } else {
        logApiRequest(

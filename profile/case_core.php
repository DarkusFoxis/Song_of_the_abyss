...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(!$conn){
        ...
        if ($user['donate'] > 0) {
            $xp_bonus = ceil($user['donate'] / 25);
            $coin_bonus = ceil($user['donate'] / 250);
            $petal_bonus = ceil($user['donate'] / 750);
            $kase_bonus = ceil($user['donate'] / 3500);
        }
                    ...
                    switch ($randomItem['name']) {
                        case 'title':
                            ...
                            if (mysqli_num_rows($result) === 0) {
                                ...
                                $sakura = ceil((mt_rand(43, 100) * $bonus) * ceil(((10 + $inv_data['lvl']) / 10))) + $petal_bonus;
                            ...
                            break;
                        case 'coins':
                            $randomCoins = ceil((mt_rand(2, 79 * ceil(((1 + $inv_data['lvl']) / 5))) * $bonus) + $coin_bonus);
                            ...
                            break;
                        case 'exp':
                            $randomXp = ceil((mt_rand(2, 155 * ceil(((1 + $inv_data['lvl']) / 4))) * $bonus) + $xp_bonus);
                           ...
                           if (!mysqli_query($conn, $query)) {
                            ...
                            break;;
                        case 'sakura':
                            $randomSakura = ceil((mt_rand(2, 30 * ceil(((1 + $inv_data['lvl']) / 5))) * $bonus) + $petal_bonus);
                            ...
                            break;
                        case 'treasures':
                            $randomSakura = ceil((mt_rand(2, 10 * ceil(((1 + $inv_data['lvl']) / 5))) * $bonus) + $petal_bonus);
                            $randomXp = ceil((mt_rand(2, 100 * ceil(((1 + $inv_data['lvl']) / 5))) * $bonus) + $xp_bonus);
                            $randomCoins = ceil((mt_rand(2, 55 * ceil(((1 + $inv_data['lvl']) / 5))) * $bonus) + $coin_bonus);
                            $query = "UPDATE `invent` SET `sakura`= `sakura` + '$randomSakura', `xp`= `xp` + '$randomXp', `coins`= `coins` + '$randomCoins' WHERE `id_user` = '$userId'";
                            if (!mysqli_query($conn, $query)) {
                                echo 'Произошла ошибка: '. mysqli_error($conn);
                            } else {
                                echo 'Вы нашли сокровище! Вы получили: ' . $randomSakura . ' лепестков сакуры, ' . $randomXp . ' опыта, и ' . $randomCoins . ' монет!';
                            }
                            $query = "UPDATE `invent` SET `kase` = `kase` - '1'  WHERE `id_user` = '$userId'";
                            mysqli_query($conn, $query);
                            break;

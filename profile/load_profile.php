...
if ($row = $result->fetch_assoc()) {
    ...
    $title_sql = "SELECT t.title, i.lvl, i.xp, i.xp_max, i.coins, i.kase, i.sakura FROM invent i LEFT JOIN title t ON i.id_title = t.id_title WHERE i.id_user = ?";
    ...
    $profile_html .= '</div>';
    $profile_html .= '<div class="profile-stat">';
    $profile_html .= '<span>Лепестки</span>';
    $profile_html .= '<span>' . ($title_data['sakura'] ?? 0) . '</span>';
    $profile_html .= '</div>';
    $profile_html .= '<div class="profile-stat">';
    $profile_html .= '<span>Кейсы</span>';
    $profile_html .= '<span>' . ($title_data['kase'] ?? 0) . '</span>';
    $profile_html .= '</div>';
    $profile_html .= '</div>';
    ...
    if ($row["lvl"] < "1") {
        ...
        $profile_html .= '<p>Аккаунт был ограничен по причине:</p>';
    ...
    if (isset($row_perm['lvl']) && $row_perm['lvl'] == "6") {
        ...
        if ($row["lvl"] == "0") {
            ...
            $profile_html .= '<input type="text" maxlength="250" id="reason" name="reason" placeholder="Причина блокировки" required>';
            $profile_html .= '<button type="submit"><i class="fas fa-ban"></i> Заблокировать</button>';
            $profile_html .= '</form>';

            $profile_html .= '<form class="group-form" id="switch_group">';
            $profile_html .= '<input type="hidden" name="user_id" value="' . $row["id"] . '">';
            $profile_html .= '<select name="group" id="group">';
            ...
            $profile_html .= '<option value="DEV"' . ($row["permissions"] == "DEV" ? " selected" : "") . '>DEV</option>';
            $profile_html .= '<option value="DELETED"' . ($row["permissions"] == "DELETED" ? " selected" : "") . '>DELETED</option>';
            $profile_html .= '</select>';

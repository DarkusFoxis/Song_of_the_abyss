<?php
require_once __DIR__ . '/../template/auth.php';

auth_start_session();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_once __DIR__ . '/../template/conn.php';

    $conn = mysqli_connect($host, $log, $password_sql, $database);
    if (!$conn) {
        echo 'Ошибка соединения.';
        exit;
    }

    $ip_address = $_SERVER['REMOTE_ADDR'];
    $login = htmlspecialchars(mysqli_real_escape_string($conn, $_POST['login'] ?? ''));
    $password = htmlspecialchars(mysqli_real_escape_string($conn, $_POST['password'] ?? ''));

    $user_query = "SELECT * FROM users WHERE login = '$login'";
    $result = $conn->query($user_query);

    $black_list = "SELECT * FROM black_ip WHERE ip = '$ip_address'";
    $black_data = $conn->query($black_list);

    if ($result && $result->num_rows > 0) {
        $user = $result->fetch_assoc();
        $password_bd = $user['password'];

        if (password_verify($password, $password_bd)) {
            $ip_data = $user['ip'];
            $account_ip = "SELECT * FROM black_ip WHERE ip = '$ip_data'";
            $result2 = $conn->query($account_ip);

            if (($result2 && $result2->num_rows > 0) || ($black_data && $black_data->num_rows > 0)) {
                $_SESSION['error'] = 'Доступ к аккаунту ограничен.';
                $conn->query("UPDATE users SET permissions = 'BANNED', reason = 'Ограничение доступа к сервису' WHERE login = '$login'");
                mysqli_close($conn);
                header('Location: main');
                exit;
            }

            $_SESSION['great'] = 'Вход успешен!';
            $conn->query("UPDATE users SET ip = '$ip_address', last_login = NOW() WHERE login = '$login'");

            if (!auth_login_user($conn, $user)) {
                $_SESSION['error'] = 'Не удалось выдать токен авторизации.';
                mysqli_close($conn);
                header('Location: main');
                exit;
            }

            mysqli_close($conn);
            header('Location: ' . auth_get_redirect_target('main'));
            exit;
        }

        $_SESSION['error'] = 'Неверный логин или пароль.';
    } else {
        $_SESSION['error'] = 'Неверный логин или пароль.';
    }

    mysqli_close($conn);
    header('Location: main');
    exit;
}

exit;

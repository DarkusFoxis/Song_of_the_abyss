...
<input type="hidden" name="action" value="new_password">
            ...
            <section id="titles" class="settings-card hidden">
                <h3><span class="card-icon">🏆</span> Титулы</h3>
                ...
                if (mysqli_num_rows($invent_result) != 0) {
                    ...
                    echo '<input type="text" id="titleSearch" class="form-control search-input" placeholder="Поиск титулов..." autocomplete="off">';
                    echo '<button type="button" id="clearSearch" class="clear-search" style="display:none;">✕</button>';
                    echo '</div>';
                    echo '</div>';
                    ...
                    if (mysqli_num_rows($title_result) != 0) {
                        while ($title = mysqli_fetch_assoc($title_result)) {
                            echo '<div class="radio-item title-item" data-title="' . htmlspecialchars($title['title']) . '">';
                            echo '<input type="radio" id="title' . $title['id_title'] . '" name="title" value="' . $title['id_title'] . '"';
                            if ($title['id_title'] == $invent['id_title']) {
                                echo ' checked';
                            }
                            echo '>';
                    ...
                    echo '<div class="radio-item title-item" data-title="Без титула">';
                    echo '<input type="radio" id="title0" name="title" value="NULL"';
                    if ($invent['id_title'] == NULL) {
                        echo ' checked';
                    }
                    echo '>';
                    echo '<label for="title0">Без титула</label>';
                    echo '</div>';

                    echo '</div>';

                    echo '<div id="noTitlesFound" class="no-results" style="display:none;">';
                    echo '<p>🔍 Титулы не найдены</p>';
                    echo '</div>';

                    echo '<button type="submit" name="submit" class="btn"><span class="btn-icon">💾</span> Сохранить</button>';
                    echo '</form>';
                } else {
                    echo '<div class="warning-box"><p>У вас пока нет доступных титулов.</p></div>';
                }
                ?>
            </section>

...
$sql = "INSERT INTO story (id_user, title, description, age_limit, story, icon, data) VALUES ($userId, '$title', '$description', $ageLimit, '$storyContent', '$iconPath', NOW())";
...
if ($conn->query($sql)) {
    ...
    if ($conn->query($query_sql)) {
        ...
        $conn->query($tonen_add_sql);
        $conn->query($tonen_sql);
    } else {

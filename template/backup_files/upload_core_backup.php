...
// Проверка переносов строк.
    ...
    if ($user['lvl'] < 3) {
        $coin = 250;
        if ($inv_data['coins'] < $coin) {
            echo "Недостаточно монет. Нужно: $coin, у вас: " . $inv_data['coins'];
            exit;
        }
        $sql = "UPDATE invent SET coins = coins - $coin WHERE id_user = '$userId'";
        if (!mysqli_query($conn, $sql)) {
            echo "Ошибка списания монет: " . mysqli_error($conn);
            exit;
        }
    }

...
function add_stikers ($conn, $userId, $setRar)
{
    ...
    if (($stiker_rand <= 77 && $stiker_rar == NULL) || $stiker_rar == "com") {
        $stiker_list = ["avrora_default","darkus_default","minami_default","misuki_default","imran_default","karyl","jahy","mon3tr","homura","kinako"];
        ...
        $stiker_list = ["avrora_halloween","avrora_reburn","misuki","old_avrora","school_avrora","school_darkus","imran_school","darkolefox","amagi"];
        ...
        $stiker_list = ["misuki","darkolefox_and_avrora","darkusfoxis","kaltsit","shiro","raphtalia","darkolefox","izuna","sameko_saba"];
        $description_list = ["Мизуки любит такояки. Это обычное японское блюдо, которое для неё готовила её бабушка.","Когда Аврора стала помощницей DarkOleFoxa, в бездне поначалу считали её дочерью.","DarkusFoxis - создатель проекта SotA.", $_SESSION['username'] . ", вам необходимо срочно поставить укол. Больно не будет.", "Широ очень любит купаться, но не любит убирать пену после себя.", "Рафталия сама попала в новый мир, хоть и никогда не планировала.", "Дарколефокс давно живёт в бездне. Он ей и дал жизнь, чуть не погибнув.", "Новый год к нам мчится, скоро всё случится...", "Встречайте новенькую рыбкокошечку!"];
        $stiker_rarity = "epic";
        $stiker_full_rarity = "эпический";
    } else if (($stiker_rand >= 97 && $stiker_rand <= 99 && $stiker_rar == NULL) || $stiker_rar == "leg"){
        $stiker_list = ["ahri","ahri2","karyl","hoshino","miku","blet"];
        $description_list = ["Ари изначально не понимала, зачем её забрали из её мира (Так захотел Оригинал).","Ари не думала, что сможет стать сильнее, и приручить драконов стихий.","Кяру не могла понять, как её магия работала в этом мире, но быстро освоилась.","Даркус был рад побывать на её концерте. Ему нравится её песни, и не только песни, если вы понимаете.","Даркус часто думал о том, чтобы оживить Хатцуне Мику, а потом встретил Неко Саму, и как-то забыл.", "Пледа нет... Всмысле нет... Так, блЭтЪ?"];
        $stiker_rarity = "leg";
        $stiker_full_rarity = "легендарный";
    } else {
        $stiker_list = ["avrora","karyl","frostnova","jahy","texas","mon3tr"];
        $description_list = ["Аврора любит свою новую жизнь. Она ничего не хочет менять в ней. Она счастлива.","Кяру считала, что в этом мире нет жуков. Попытавшись выйти из дома 12 мая, она пообещала больше никогда не выходить на улицу.", $_SESSION['username'] . " не ожидал увидеть ФростНову, ведь она умерла прямо у него на руках... Кажется, Однорогий хранит много секретов...", "Джахи готова к новым открытиям в этой вселенной! А ты?", "Техас уже есть арбуз. А ты купил его?", "Mon3tr loves you, and sends you a sweet heart♡."];
        $stiker_rarity = "myst";

...
try {
    ...
    // === ИЗМЕНЕНИЕ: MIME-тип берём из исходника, а не хардкодим 'video/webm' ===
    $sourceMime = (string)$sourceStored['mime'];
    $sourceSize = (int)$sourceStored['size'];
    $videoMime = $sourceMime;
    ...
    video_write_upload_status($uploadToken, [
        ...
        'status' => 'processing',
        'progress' => 40,
        'message' => 'Черновик создан, файл перемещается в хранилище...',
        'user_id' => (int)$viewer['id'],
        'video_id' => $videoId,
        'watch_url' => video_watch_url($videoId),
    ]);

    // === ИЗМЕНЕНИЕ: Вместо перекодирования — прямое перемещение исходника в processed ===
    $sourceExt = $allowedVideoMap[$sourceMime] ?? 'mp4';
    $finalVideoName = video_random_name('video', $sourceExt);
    $finalVideoPath = rtrim($processedDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $finalVideoName;

    if (!rename((string)$sourceStored['path'], $finalVideoPath)) {
        // Fallback: copy + delete, если rename не сработает (разные файловые системы)
        if (!copy((string)$sourceStored['path'], $finalVideoPath)) {
            throw new RuntimeException('Не удалось переместить загруженный файл в хранилище.');
        }
        @unlink((string)$sourceStored['path']);
    }
    // Исходник перемещён — очищаем ссылку, чтобы catch не удалил файл
    $sourceStored = null;
    ...
    video_write_upload_status($uploadToken, [
        ...
        'status' => 'processing',
        'progress' => 55,
        'message' => 'Файл сохранён, извлекаются метаданные...',
        'user_id' => (int)$viewer['id'],
        'video_id' => $videoId,
        'watch_url' => video_watch_url($videoId),
    ]);

    // === ИЗМЕНЕНИЕ: Извлекаем метаданные ffprobe без перекодирования ===
    $meta = video_probe_metadata($finalVideoPath);

    $progressStmt = $conn->prepare('UPDATE video_posts SET processing_progress = 65 WHERE id = ?');
    ...
    video_write_upload_status($uploadToken, [
        ...
        'status' => 'processing',
        'progress' => 70,
        'message' => 'Метаданные получены, подготавливается обложка...',
        'user_id' => (int)$viewer['id'],
        'video_id' => $videoId,
        'watch_url' => video_watch_url($videoId),
    ]);
    ...
    if (isset($_FILES['cover']) && is_array($_FILES['cover']) && !empty($_FILES['cover']['name'])) {
        ...
        video_generate_frame_cover($finalVideoPath, $coverPath, (float)($meta['duration'] ?? 0));
        $coverMime = 'image/jpeg';
    }

    // === ИЗМЕНЕНИЕ: Убрано закрытие/переоткрытие БД — нет длительной операции ===
    ...
    $finalSize = (int)($meta['size'] ?? filesize($finalVideoPath));
    ...
    // === ИЗМЕНЕНИЕ: Убрана очистка исходника — он уже перемещён в processed ===
